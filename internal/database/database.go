...
func (db *DB) createTables() error {
	...
	if _, err := db.conn.Exec(questionsQuery); err != nil {
		return fmt.Errorf("failed to create daily_questions table: %w", err)
	}
...
func (db *DB) AddQuestion(question, authorID, authorName, messageID, channelID string) (int64, error) {
	...
	query := `INSERT INTO daily_questions (question, author_id, author_name, message_id, channel_id) VALUES (?, ?, ?, ?, ?)`
...
func (db *DB) GetQuestionByMessageID(messageID string) (*Question, error) {
	query := `SELECT id, question, author_id, author_name, created_at, times_asked, last_asked_at, message_id, channel_id, approval_status, approval_message_id, approved_by, approved_at FROM daily_questions WHERE message_id = ?`
	var q Question
...
func (db *DB) ApproveQuestion(questionID int, approverID string) error {
    ...
    query := `UPDATE daily_questions SET approval_status = 'approved', approved_by = ?, approved_at = NOW() WHERE id = ?`
...
func (db *DB) GetPendingQuestion() (*Question, error) {
    ...
    query := `SELECT id, question, author_id, author_name, created_at, times_asked, last_asked_at, message_id, channel_id, approval_status, approval_message_id, approved_by, approved_at FROM daily_questions WHERE approval_status = 'pending' ORDER BY created_at ASC LIMIT 1`
    var q Question
...
func (db *DB) UpdateApprovalMessageID(questionID int, approvalMessageID string) error {
	...
	query := `UPDATE daily_questions SET approval_message_id = ? WHERE id = ?`
...
func (db *DB) GetQuestionByApprovalMessageID(approvalMessageID string) (*Question, error) {
	...
	query := `SELECT id, question, author_id, author_name, created_at, times_asked, last_asked_at, message_id, channel_id, approval_status, approval_message_id, approved_by, approved_at FROM daily_questions WHERE approval_message_id = ?`
	var q Question
...
func (db *DB) GetPendingQuestionByID(questionID int) (*Question, error) {
	...
	query := `SELECT id, question, author_id, author_name, created_at, times_asked, last_asked_at, message_id, channel_id, approval_status, approval_message_id, approved_by, approved_at FROM daily_questions WHERE id = ? AND approval_status = 'pending'`
	var q Question
...
func (db *DB) GetApprovalStats() (int, int, int, error) {
	...
	// Get pending count
	pendingQuery := `SELECT COUNT(*) FROM daily_questions WHERE approval_status = 'pending'`
	...
	// Get approved count
	approvedQuery := `SELECT COUNT(*) FROM daily_questions WHERE approval_status = 'approved'`
	...
	// Get rejected count
	rejectedQuery := `SELECT COUNT(*) FROM daily_questions WHERE approval_status = 'rejected'`
...
func (db *DB) IncrementQuestionUsage(questionID int) error {
	...
	query := `UPDATE daily_questions SET times_asked = times_asked + 1, last_asked_at = NOW() WHERE id = ?`
...
func (db *DB) GetApprovedQuestionStats() (int, int, int, error) {
	...
	totalApprovedQuery := `SELECT COUNT(*) FROM daily_questions WHERE approval_status = 'approved'`
	...
	totalAskedQuery := `SELECT COALESCE(SUM(times_asked), 0) FROM daily_questions WHERE approval_status = 'approved'`
	...
	minAskedQuery := `SELECT COALESCE(MIN(times_asked), 0) FROM daily_questions WHERE approval_status = 'approved'`
