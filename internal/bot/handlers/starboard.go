...
// HandleStarReaction handles star reactions for the starboard feature.
func (h *Handler) HandleStarReaction(s *discordgo.Session, r *discordgo.MessageReactionAdd) {
	if r.UserID == s.State.User.ID { // Ignore bot's own reactions
		return
	}
	// Don't process reactions in the starboard channel itself
	if r.ChannelID == h.Bot.GetConfig().Starboard.ChannelID {
		return
	}

	if r.Emoji.Name == "⭐" { // Check for star emoji
		// Fetch message
		...
		// Log for debugging
		log.Printf("Message %s in channel %s has %d stars (threshold: %d)", r.MessageID, r.ChannelID, stars, h.Bot.GetConfig().Starboard.Threshold)

		if stars >= h.Bot.GetConfig().Starboard.Threshold {
			...
			_, err := s.ChannelMessageSendEmbed(h.Bot.GetConfig().Starboard.ChannelID, embed)
			if err != nil {
